...
case class Factor(rows: List[FactorRow]) {
  ...
  def mult(factor: Factor) : Factor = {
    ...
    Factor(rows.map((row) => FactorRow(row.realization, row.value * factor.value_at(row.realization))))
  }
...
class FactorSpec extends FlatSpec with Matchers {
  ...
  //    Factor(List(row)).variables() should be (List(a, b))
  //  }
  //
  ...
  //
  //    val facty = Factor(List(row_false, row_true))
  ...
  //
  //    val facty = Factor(List(row00, row01, row10, row11))
  //    facty.value_at(Realization(Map(a->0))) should be (1 - p_true)
  //  }
//
...
//    val facty = Factor(List(row0, row1))
...
//    val other_facty = Factor(List(row0, row1))
//
