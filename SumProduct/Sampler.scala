...
case class LogSliceSampler(distribution: Distribution, previous_value: Double) extends Sampler {
  ...
  def choose_height: Double = {
    current_prob - breeze.stats.distributions.Exponential(1).draw()
  }
  ...
  def sample(n: Int, current_result: List[Double] = List()) : List[Double] = {
    if (n == 0) current_result
      ...
      val next_sample = LogSliceSampler(distribution, prev_value).draw
      sample(n - 1, List(next_sample) ++ current_result)
    }
  }
}

class LogSliceSamplerSpec extends FlatSpec with Matchers {
  "A log slice sampler " should " closely approximate the true mean" in {
    val variable = VariableFactory("a")
    val true_mean = 7.0
    val true_stdev = 0.5

    def f(realization: Realization): Double = scala.math.log(Gaussian(true_mean, true_stdev).pdf(realization.realization.values.head))
    val ff : (Realization => Double) = f
    val distr = DistributionFactory(variable, ff)
    val sampler = LogSliceSampler(distr, 1.0)
